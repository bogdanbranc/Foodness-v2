...
export default function Meniu() {
    ...
    const [breakfastMenus, setBreakastMenus] = useState([])//variabile care se modifica si modificarea lor duce la rerendering(se actualizeaza in timp real)
    ...
    const handleChange = (event, newAlignment) => {
     ...
     {/* incepe prin a copia cartul care initial e gol,daca e gol adauga primul element default fara nici o verificare,verifica ca produsu selectat curent pentru adaugarea in cart,dupa data si tip iar daca sunt selectate 2 produse de acelasi tip,al doilea produs in inlocuieste pe primul selectat valabil pentru aceeasi zi calendaristica */}
    ...
    useEffect(() => {       //preia din baza de date toate meniurile
        getMenus();
    }, [])



    const getMenus = async () => {   //functia care preia care se seteaza cand deschidem pagina de meniu,functie asincrona fiinda ca operatiunea de preluare dureaza pana se preia din baza de date
        const data = await MenuDataService.getAllMenus();  //se salveaza datele din tabela food prin metoda getAllMenu care se afla in controler food.services
    ...
    function dateMaker(date) {   //functia dateMaker ia data selectata in calendar si in functie de tipul de abonament selectat genereaza urmatoarele zile automat pentru configurare si livrare
        const dates = []
    ...
    return (
        <> {/* className foloseste biblioteca Tailwind */}
            <div className="flex flex-row">
                ...
                <div className="flex flex-col gap-10 flex-spx-4 py-4 mx-auto  sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 ">
                    ...
                    <div className="grid max-w-md gap-10  lg:max-w-screen-lg sm:row-gap-10 lg:grid-cols-3 xl:max-w-screen-lg sm:mx-auto">
                        ...
                        <div className="flex flex-col p-8 transition-shadow duration-300 bg-white border rounded shadow-sm sm:items-center hover:shadow">  {/* cardul pentru meniu */}
                    ...
                    {tipAbonament && <div className="grid max-w-md lg:max-w-screen-lg xl:max-w-screen-lg sm:mx-auto">
                        ...
                        <LocalizationProvider dateAdapter={AdapterDateFns}>   {/* Adaptor pentru timp pentru a afla ziua curenta pentru a selecta data din calendar */}
                            <DatePicker
                    ...
                    {date && <div className="grid max-w-md lg:max-w-screen-lg xl:max-w-screen-lg sm:mx-auto">    {/* Componente de Material UI,in momentul cand selectam o optiune ramane selectat pentru casutele cu dati p*/}
                        <ToggleButtonGroup
                            color="primary"
                            value={alignment}
                            exclusive
                    ...
                    {currentItemDate && <div className='grid  max-w-md gap-10  lg:max-w-screen-lg sm:row-gap-10 lg:grid-cols-3 xl:max-w-screen-lg sm:mx-auto'> {/* Configurarea meniurilor */}
                    ...
                    {currentItemDate && <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl ">  {/* Daca exista minim un preparat selectat creaza partea de front end pentru el(conditional rendering) */}
                        <div className="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            ...
                            <div className="mt-8">
                                <div className="flow-root">
                                    <ul role="list" className="-my-6 divide-y divide-gray-200"> {/* cart semnifica o lista cu preparatele selectate,afisand dinamic tot ce este selectat in timp real */}
                                        {cart.map((product) => (                      
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        console.log(tipAbonament * 4) 
                                         {/* adauga cartul configurat stateul aplicatiei,ajutand si pentru indexarea numarului de produse in cart */}
